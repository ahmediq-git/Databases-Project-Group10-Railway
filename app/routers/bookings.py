...
def get_booking(id: int, current_user: int = Depends(oauth2.get_current_user)):
    ...
    conn, cursor = get_db()
    sql_query = '''
                SELECT users.user_id, seat_id, fname, lname, phone_num, arrival_time,  departure_time, departure_city, routes.arrival_city
                FROM bookings
...
def get_booking_status(id: int, current_user: int = Depends(oauth2.get_current_user)):
...
@router.get("/travel-history", response_model=List[schemas.BookingHistory]) # 
def get_booking(current_user: int = Depends(oauth2.get_current_user)):
   
    # posts = db.query(models.Post).filter(models.Post.title.contains(search)).limit(limit).offset(skip).all()
    conn, cursor = get_db()
    sql_query = '''
                SELECT arrival_city, departure_city, departure_date
                FROM bookings
                JOIN schedule ON bookings.schedule_id = schedule.schedule_id
                JOIN routes ON schedule.route_id = routes.route_id
...
@router.post("/add-booking", status_code=status.HTTP_201_CREATED) # , response_model=schemas.BookingOut
